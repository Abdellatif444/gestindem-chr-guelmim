...
def nav_html(active_label):
    ...
    for item in NAV:
        if item[0] == "group":
            ...
            first_group = False
            out.append('      <div style="font-size: 10px; font-weight: 600; letter-spacing: 1.5px; color: #5F776E; padding: %s;">%s%s</div>' % (pad, KHATAM8, item[1]))
            continue
        ...
        <span style="min-width: 18px; height: 18px; border-radius: 9px; background: #B98A2F; color: #0B2E29; font-size: 11px; font-weight: 700; display: flex; align-items: center; justify-content: center; padding: 0 5px; box-sizing: border-box;">%s</span>
  ...
  <div style="width: 236px; height: 900px; background: #0B2E29; display: flex; flex-direction: column; box-sizing: border-box; padding: 20px 14px; flex-shrink: 0;">
    <div style="display: flex; align-items: center; gap: 11px; padding: 4px 8px 18px 8px;">
      ...
      <div style="display: flex; flex-direction: column; gap: 1px;">
        ...
        <span style="font-size: 10px; color: #7E968D;">CHR Guelmim</span>
      </div>
    </div>
...
{nav}
    ...
    <div style="padding: 10px 10px 0; border-top: 1px solid rgba(255,255,255,0.09); font-size: 10.5px; color: #5F776E;">v1.0 · Dernière sauvegarde 02:00</div>
  </div>
    ...
    <div style="height: 60px; background: #FFFFFF; border-bottom: 1px solid #DBE3DE; display: flex; align-items: center; justify-content: space-between; padding: 0 24px; box-sizing: border-box; flex-shrink: 0;">
      ...
      <div style="display: flex; align-items: center; gap: 16px;">
        <div style="display: flex; align-items: center; gap: 8px; border: 1px solid #DBE3DE; border-radius: 8px; padding: 7px 12px; width: 240px; box-sizing: border-box;">
          <svg width="14" height="14" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg"><circle cx="7" cy="7" r="4.5" stroke="#8A968F" stroke-width="1.5"></circle><path d="M10.5 10.5 L14 14" stroke="#8A968F" stroke-width="1.5" stroke-linecap="round"></path></svg>
          <span style="font-size: 13px; color: #8A968F;">{search}</span>
        </div>
        <div style="position: relative;">
          ...
          <span style="position: absolute; top: -2px; right: -3px; width: 8px; height: 8px; border-radius: 4px; background: #B3261E; border: 1.5px solid #FFFFFF;"></span>
        </div>
        <div style="display: flex; align-items: center; gap: 10px; padding-left: 16px; border-left: 1px solid #DBE3DE;">
          ...
          <div style="display: flex; flex-direction: column; gap: 1px;">
            ...
            <span style="font-size: 11px; color: #61706A;">{role}</span>
          </div>
        </div>
      </div>
    </div>
...
def sel(label, value):
    return ('<div style="display: flex; align-items: center; gap: 7px; background: #FFFFFF; border: 1px solid #DBE3DE; border-radius: 8px; padding: 8px 12px;">'
        '<span style="font-size: 12px; color: #8A968F;">%s</span>'
...
def badge(text, kind):
    ...
    bg, fg = styles[kind]
    return '<span style="background: %s; color: %s; font-size: 11.5px; font-weight: 600; padding: 3px 9px; border-radius: 20px;">%s</span>' % (bg, fg, text)
...
PERSONNEL = page_header(
    ...
    btn_secondary("Importer depuis Excel", IMPORT_ICON) + btn_primary("Nouvel agent")) + """
      ...
      <div style="display: flex; align-items: center; gap: 10px;">
        ...
        <span style="font-size: 12.5px; color: #8A968F; margin-left: auto;">248 agents · filtre : aucun</span>
      </div>
      ...
      <div style="display: flex; gap: 16px; flex-grow: 1; min-height: 0;">
        <div style="flex-grow: 1; background: #FFFFFF; border: 1px solid #DBE3DE; border-radius: 10px; padding: 16px 18px; box-sizing: border-box; display: flex; flex-direction: column; gap: 8px;">
          <div style="display: grid; grid-template-columns: 80px 160px 160px 65px 110px 70px; gap: 8px; padding: 7px 10px; font-size: 11px; font-weight: 600; letter-spacing: 0.6px; color: #8A968F; text-transform: uppercase; border-bottom: 1px solid #E9EEEA;">
...
""" + "\n".join(rows_html) + """
          ...
          <div style="margin-top: auto; display: flex; justify-content: space-between; align-items: center; padding-top: 10px; border-top: 1px solid #E9EEEA;">
            ...
            <span style="font-size: 12px; color: #8A968F;">7 agents affichés sur 248 · page 1/36</span>
          </div>
        </div>
        ...
        <div style="width: 400px; background: #FFFFFF; border: 1px solid #DBE3DE; border-radius: 10px; padding: 18px 20px; box-sizing: border-box; display: flex; flex-direction: column; gap: 14px; flex-shrink: 0;">
          ...
          <div style="background: #F6F9F7; border: 1px solid #E9EEEA; border-radius: 8px; padding: 12px 14px; display: flex; flex-direction: column; gap: 8px;">
            <span style="font-size: 11px; font-weight: 600; letter-spacing: 0.8px; color: #8A968F; text-transform: uppercase;">Août 2026</span>
...
def tree_item(indent, icon, label, count, selected=False, muted=False):
    color = "#10554A" if selected else ("#8A968F" if muted else "#182420")
    ...
    return ('<div style="display: flex; align-items: center; gap: 8px; padding: 8px 10px 8px %dpx;%s">'
        ...
        '<span style="font-size: 11.5px; color: #8A968F; margin-left: auto;">%s</span></div>') % (10 + indent * 18, bg, icon, weight, color, label, count)
    ...
    btn_primary("Nouvelle structure")) + """
      ...
      <div style="display: flex; gap: 16px; flex-grow: 1; min-height: 0;">
        <div style="width: 430px; background: #FFFFFF; border: 1px solid #DBE3DE; border-radius: 10px; padding: 16px 14px; box-sizing: border-box; display: flex; flex-direction: column; gap: 2px; flex-shrink: 0;">
          <span style="font-size: 11px; font-weight: 600; letter-spacing: 0.8px; color: #8A968F; text-transform: uppercase; padding: 0 10px 8px;">Hiérarchie régionale</span>
""" + "\n".join([
    ...
    tree_item(3, '<span style="width: 6px; height: 6px; border-radius: 3px; background: #8A968F;"></span>', "Urgences", "57 agents"),
    tree_item(3, '<span style="width: 6px; height: 6px; border-radius: 3px; background: #8A968F;"></span>', "Réanimation", "31 agents"),
    tree_item(3, '<span style="width: 6px; height: 6px; border-radius: 3px; background: #8A968F;"></span>', "Chirurgie · Maternité · Pédiatrie…", "7 autres"),
    tree_item(2, ticon("#61706A"), "CS urbain Guelmim-Centre", "23 agents"),
    tree_item(2, ticon("#61706A"), "CS rural Bouizakarne", "11 agents"),
    tree_item(1, ticon("#8A968F"), "Délégation de Tan-Tan", "8 structures", muted=True),
    tree_item(1, ticon("#8A968F"), "Délégation de Sidi Ifni", "7 structures", muted=True),
    tree_item(1, ticon("#8A968F"), "Délégation d'Assa-Zag", "5 structures", muted=True),
]) + """
        </div>
        ...
        <div style="flex-grow: 1; display: flex; flex-direction: column; gap: 14px; min-width: 0;">
          <div style="background: #FFFFFF; border: 1px solid #DBE3DE; border-radius: 10px; padding: 18px 20px; display: flex; flex-direction: column; gap: 12px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 12px;">
              <div style="background: #F6F9F7; border: 1px solid #E9EEEA; border-radius: 8px; padding: 12px 14px; display: flex; flex-direction: column; gap: 4px;">
                <span style="font-size: 11.5px; color: #61706A;">Agents rattachés</span><span style="font-size: 20px; font-weight: 700;">248</span>
              </div>
              <div style="background: #F6F9F7; border: 1px solid #E9EEEA; border-radius: 8px; padding: 12px 14px; display: flex; flex-direction: column; gap: 4px;">
                <span style="font-size: 11.5px; color: #61706A;">Utilisateurs</span><span style="font-size: 20px; font-weight: 700;">9</span>
              </div>
              <div style="background: #F6F9F7; border: 1px solid #E9EEEA; border-radius: 8px; padding: 12px 14px; display: flex; flex-direction: column; gap: 4px;">
                <span style="font-size: 11.5px; color: #61706A;">Services</span><span style="font-size: 20px; font-weight: 700;">9</span>
              </div>
            </div>
          </div>
          ...
          <div style="background: #FFFFFF; border: 1px solid #DBE3DE; border-radius: 10px; padding: 18px 20px; display: flex; flex-direction: column; gap: 10px; flex-grow: 1;">
            ...
            <div style="display: grid; grid-template-columns: 130px 1fr 190px 130px; gap: 10px; padding: 7px 10px; font-size: 11px; font-weight: 600; letter-spacing: 0.6px; color: #8A968F; text-transform: uppercase; border-bottom: 1px solid #E9EEEA;">
            ...
            <div style="margin-top: auto; display: flex; align-items: flex-start; gap: 9px; background: #F6F9F7; border: 1px solid #E9EEEA; border-radius: 8px; padding: 11px 13px;">
              ...
              <span style="font-size: 12px; color: #3A5C52; line-height: 1.5;">Les formulaires et paramètres de saisie sont adaptables par structure (champs visibles, services, types d'indemnité activés) — voir Administration.</span>
            </div>
          </div>
        </div>
      </div>"""
...
BAREMES = page_header(
    ...
    btn_secondary("Exporter") + btn_primary("Nouvelle version")) + """

      <div style="display: flex; gap: 8px;">
        <span style="padding: 8px 16px; background: #10554A; color: #FFFFFF; border-radius: 8px; font-size: 13px; font-weight: 600;">Garde · Astreinte · Permanence</span>
        <span style="padding: 8px 16px; background: #FFFFFF; color: #61706A; border: 1px solid #DBE3DE; border-radius: 8px; font-size: 13px; font-weight: 500;">Déplacement</span>
      </div>
      ...
      <div style="display: flex; gap: 16px; flex-grow: 1; min-height: 0;">
        <div style="flex-grow: 1; display: flex; flex-direction: column; gap: 14px; min-width: 0;">
          <div style="background: #FFFFFF; border: 1px solid #DBE3DE; border-radius: 10px; padding: 16px 18px; display: flex; flex-direction: column; gap: 10px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 110px 110px 120px; gap: 10px; padding: 7px 10px; font-size: 11px; font-weight: 600; letter-spacing: 0.6px; color: #8A968F; text-transform: uppercase; border-bottom: 1px solid #E9EEEA;">
          ...
          <div style="background: #FFFFFF; border: 1px solid #DBE3DE; border-radius: 10px; padding: 16px 18px; display: flex; flex-direction: column; gap: 10px; flex-grow: 1;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 170px 160px; gap: 10px; padding: 7px 10px; font-size: 11px; font-weight: 600; letter-spacing: 0.6px; color: #8A968F; text-transform: uppercase; border-bottom: 1px solid #E9EEEA;">
            ...
            <div style="display: flex; gap: 10px; margin-top: 4px;">
              <div style="flex-grow: 1; background: #F6F9F7; border: 1px solid #E9EEEA; border-radius: 8px; padding: 10px 13px; display: flex; flex-direction: column; gap: 6px;">
                <span style="font-size: 11px; font-weight: 600; letter-spacing: 0.8px; color: #8A968F; text-transform: uppercase;">Coefficients de distance (paramétrables)</span>
        ...
        <div style="width: 400px; background: #FFFFFF; border: 1px solid #DBE3DE; border-radius: 10px; padding: 18px 20px; box-sizing: border-box; display: flex; flex-direction: column; gap: 12px; flex-shrink: 0;">
          ...
          <div style="display: flex; flex-direction: column; gap: 0;">
            ...
            <div style="display: flex; gap: 12px; padding: 10px 0; border-bottom: 1px solid #EDF1EE;">
              ...
              <div style="display: flex; flex-direction: column; gap: 3px;">
                ...
                <span style="font-size: 12px; color: #8A968F;">Effet 01/01/2026 → 30/06/2026</span>
              </div>
            </div>
            <div style="display: flex; gap: 12px; padding: 10px 0;">
              ...
              <div style="display: flex; flex-direction: column; gap: 3px;">
                ...
                <span style="font-size: 12px; color: #8A968F;">Effet 01/06/2025 → 31/12/2025</span>
              </div>
            </div>
          </div>
          <div style="margin-top: auto; display: flex; align-items: flex-start; gap: 9px; background: #FBEEDC; border: 1px solid #F2DDBB; border-radius: 8px; padding: 11px 13px;">
            ...
            <span style="font-size: 12px; color: #7A5210; line-height: 1.5;">Le calcul applique toujours le barème en vigueur à la date du planning ou de la mission — jamais le barème courant. Valeurs de démonstration : à remplacer par les taux réglementaires.</span>
          </div>
        </div>
      </div>"""
...
MISSIONS = page_header(
    ...
    btn_secondary("Exporter") + btn_primary("Nouvel ordre de mission")) + """
      ...
      <div style="display: flex; gap: 16px; flex-grow: 1; min-height: 0;">
        <div style="flex-grow: 1; background: #FFFFFF; border: 1px solid #DBE3DE; border-radius: 10px; padding: 16px 18px; box-sizing: border-box; display: flex; flex-direction: column; gap: 8px;">
          <div style="display: grid; grid-template-columns: 100px 135px 110px 70px 55px 70px 100px; gap: 8px; padding: 7px 10px; font-size: 11px; font-weight: 600; letter-spacing: 0.6px; color: #8A968F; text-transform: uppercase; border-bottom: 1px solid #E9EEEA;">
          ...
          <div style="margin-top: auto; display: flex; justify-content: space-between; align-items: center; padding-top: 10px; border-top: 1px solid #E9EEEA;">
            <span style="font-size: 12px; color: #8A968F;">6 missions affichées sur 14 — Août 2026</span>
            <a href="#" style="font-size: 12.5px; font-weight: 600;">Historique complet des déplacements</a>
          </div>
        </div>
        ...
        <div style="width: 400px; background: #FFFFFF; border: 1px solid #DBE3DE; border-radius: 10px; padding: 18px 20px; box-sizing: border-box; display: flex; flex-direction: column; gap: 13px; flex-shrink: 0;">
          ...
          <div style="background: #F6F9F7; border: 1px solid #E9EEEA; border-radius: 8px; padding: 12px 14px; display: flex; flex-direction: column; gap: 7px;">
            <span style="font-size: 11px; font-weight: 600; letter-spacing: 0.8px; color: #8A968F; text-transform: uppercase;">Indemnité prévisionnelle — détail du calcul</span>
          ...
          <div style="display: flex; flex-direction: column; gap: 8px;">
            <span style="font-size: 12px; font-weight: 600; letter-spacing: 0.8px; color: #8A968F; text-transform: uppercase;">Historique</span>
...
SCREENS = [
    ("Personnel.dc.html", "Personnel", PERSONNEL, "Rechercher un matricule, un nom…", "HB", "H. Bouzid", "Administrateur"),
